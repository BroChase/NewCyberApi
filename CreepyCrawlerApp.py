...
from cyberapi import makemenu
import requests
from cyberapi import analysis
...
class cyberapi(Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.menu = makemenu.MenuMaker2000(self).createMenu()
        ...
        self.frames = {}
        for F in (SearchFrame, searchtitlekeyword):       #The two windows used in program sets the page
            page_name = F.__name__
            frame = F(parent=container, controller=self)
            self.frames[page_name] = frame
            frame.grid(row=0, column=0, sticky="nsew")

        self.show_frame("SearchFrame")          #call show_frame to display the welcome window
...
class SearchFrame(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.topicsHead = Label(self, font="times 16 underline")
        self.topics = Label(self, wraplength=500, font='times 14')
    ...
    def filter_op(self):
        if self.var.get():
            ...
            self.var2 = IntVar()
            self.var2.set(1)
            self.fsub_nv = Radiobutton(self, text ="Don't Care", variable=self.var2, value=1)
            self.fsub_gt = Radiobutton(self, text='More Subjective', variable=self.var2, value=2)
            self.fsub_lt = Radiobutton(self, text='More Objective', variable=self.var2, value=3)
    ...
    def tkwsearch(self):
        ...
        self.ent_title.bind('<Key>', lambda event: self.callEnable(event, 'tkw'))
    ...
    def tbssearch(self):
        ...
        self.ent_titleb = Entry(self, width=30, bd=2)
        self.titlelabelb = Label(self, text='Title')
        self.ent_titleb.bind('<Key>', lambda event: self.callEnable(event, 'tb'))
        self.bodylabel = Label(self, text='Body')
        self.ent_body = Entry(self, width=30, bd=2)
        self.ent_body.bind('<Key>', lambda event: self.callEnable(event, 'tb'))
        self.but_search = Button(self, text='Search', width=20, state='disable', command=lambda: self.search('http://cbrown686-test.apigee.net/cyberapi/articles?q=keywordbody&title='
                                                                                  + self.ent_titleb.get()+'&body=' + self.ent_body.get()))
        #window placments
        self.titlelabelb.grid(column=3, row=1)
        self.bodylabel.grid(column=3, row =2)
        self.ent_titleb.grid(column=4, row=1)
    ...
    def urisearch(self):
        ...
        self.ent_uri.bind('<Key>', lambda event: self.callEnable(event, 'uri'))
    ...
    def sbdsearch(self):
        ...
        self.ent_edate.bind('<Key>', lambda event: self.callEnable(event, 'date'))
    ...
    def clearshit(self):
        ...
        if hasattr(self, 'bodylabel'):
            self.titlelabelb.destroy()
            self.ent_titleb.destroy()
        ...
        if data:
            for item in data:
                ...
                self.tree.insert('', 'end', text=item['title'], values=(item['uri'], item['body'], item['title'],
                                                                        item['author'], parser.parse(item['date']).strftime('%B, %d, %Y')))

            self.tree.bind('<Double-1>', self.on_click)
            self.tree.bind('<<TreeviewSelect>>', self.on_single_click)

            self.topics.grid(column=7, row=16)
            self.topicsHead.grid(column=7, row=15)

        else:
            self.topicsHead.config(text='')
            self.topics.config(text='')
            self.topicsHead.grid_forget()
            self.topics.grid_forget()

#on_click "double clicking on the article from the tree window opens up the article to be viewed"

    def on_single_click(self, event):
        self.topicsHead.config(text="Main Subjects of Article:")
    ...
    def on_click(self, event):
        ...
        articledate.grid(column=4, row=4, sticky='E')
    ...
    def enableSearch(self, event, searchType):
        string = ''
        if searchType == 'tkw':
            string = self.ent_title.get()
        elif searchType == 'tb':
            string = self.ent_titleb.get() + ' ' + self.ent_body.get()
        elif searchType == 'uri':
            string = self.ent_uri.get()
        else:
            string = self.ent_date.get() + self.ent_edate.get()
