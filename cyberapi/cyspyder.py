...
class SearchFrame(Frame):
    ...
    def searchwindow(self):
        ...
        if not self.var.get():
            self.but_search = Button(self, text='Search', width=15, state='disable', font="Veranda 16", command=lambda: self.search(
                ...
                + self.ent_keyword.get().replace(' ', '+') + '&body=' + self.ent_keyword.get().replace(' ', '+')))
    ...
    def searchButton(self,event):
        if self.box.current() is 0:
            ...
            self.but_search.config(command=lambda: self.search(
                'http://cbrown686-test.apigee.net/cyberapi/articles?q=keywordtitle&title=' + self.ent_keyword.get().replace(' ', '+')))
        elif self.box.current() is 2:
            self.but_search.config(command=lambda: self.search(
                'http://cbrown686-test.apigee.net/cyberapi/articles?q=bodyonly&body=' + self.ent_keyword.get().replace(' ', '+')))
        elif self.box.current() is 3:
            self.but_search.config(command=lambda: self.search(
                'http://cbrown686-test.apigee.net/cyberapi/articles?q=uri&uripath='+ self.ent_keyword.get().replace(' ', '+')))
